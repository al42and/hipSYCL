...
struct DivergencePropagator {
  ...
  std::unique_ptr<ControlDivergenceDesc> computeJoinPoints() {
    ...
    for (const auto *SuccBlock : successors(&DivTermBlock)) {
      ...
      LLVM_DEBUG(dbgs() << "\tImmediate divergent loop exit: " << SuccBlock->getName() << "\n");
    }
    ...
    for (; BlockIdx >= FloorIdx; --BlockIdx) {
      ...
      if (IsLoopHeader) {
        ...
        for (const auto *BlockLoopExit : BlockLoopExits) {
          ...
          LoweredFloorIdx = std::min<int>(LoweredFloorIdx, LoopPOT.getIndexOf(*BlockLoopExit));
        }
      } else {
        // Acyclic successor case
        for (const auto *SuccBlock : successors(Block)) {
          CausedJoin |= visitEdge(*SuccBlock, *Label);
          LoweredFloorIdx = std::min<int>(LoweredFloorIdx, LoopPOT.getIndexOf(*SuccBlock));
        }
      }
...
const ControlDivergenceDesc &SyncDependenceAnalysis::getJoinBlocks(const Instruction &Term) {
  // trivial case
